        ...
        // ===================== SMOOTH SCROLL =====================
        document.querySelectorAll('a[href^="#"]').forEach(a => { a.addEventListener('click', function (e) { e.preventDefault(); hamburger.classList.remove('active'); navLinks.classList.remove('active'); const t = document.querySelector(this.getAttribute('href')); if (t) { const y = t.getBoundingClientRect().top + window.pageYOffset - headerEl.offsetHeight - 8; window.scrollTo({ top: Math.max(0, y), behavior: 'smooth' }) } }) });
        ...
        function openGames() { overlay.classList.add('open'); hamburger.classList.remove('active'); navLinks.classList.remove('active') }
        ...
        document.getElementById('settingsReset').addEventListener('click', () => {
            ['accent', 'dim', 'theme', 'bgEffect'].forEach(k => { try { localStorage.removeItem('pf_' + k) } catch (e) { } });
